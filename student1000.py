...
addrs = ['서울', '부산', '대구', '대전', '광주', '전주']

def nr(n = 4): # nr: number_random
  return "".join(random.sample(nums, n))

def ar(n = 5): # ar: alphabet_random
...
def make_birth():
  ...
  return "{}{:02d}{:02d}".format(y, m, d)
    # {02d} => 정수2자리로 표시하되, 자릿수가 부족하면 0으로 채움
...
# MySQL Connection 연결
# Docker Desktop 런닝시키고
# power shell 명령어에서 $> docker start mysql5
# workbench에서 해당 데이타베이스 작동시킨다
